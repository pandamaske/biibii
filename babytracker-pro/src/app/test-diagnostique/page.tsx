...
import { formatAge, getAgeInWeeks } from '@/lib/utils'
...
export default function ProfilePage() {
  ...
  const { currentBaby, babies, initializeData, updateBaby } = useBabyTrackerStore()
  ...
  const userStats = useMemo(() => {
    ...
    const daysSinceBirth = Math.floor((Date.now() - new Date(currentBaby.birthDate).getTime()) / (1000 * 60 * 60 * 24))
  ...
  if (!currentBaby || !userProfile || !appSettings) {
    return (
      <AppLayout 
        className="bg-gradient-to-b from-purple-400 to-white"
  ...
  return (
    ...
    >
      <div className="p-6 pb-24 space-y-6 animate-fade-in">
        {/* Profile Header */}
        <div className="bg-gradient-to-r from-purple-500 to-pink-600 rounded-3xl p-6 text-white shadow-large">
          <div className="flex items-center space-x-4 mb-6">
            ...
            <div className="flex-1">
              <h2 className="text-2xl font-bold">
                Bonjour {userProfile.preferredName} ! 👋
              </h2>
              ...
              <div className="flex items-center space-x-2 text-sm text-purple-100">
                ...
                <span>Membre depuis {new Date(userProfile.createdAt).toLocaleDateString('fr-FR')}</span>
              </div>
            </div>
          </div>
          ...
          {userStats && (
            <div className="grid grid-cols-4 gap-3">
              ...
              <div className="bg-white/20 backdrop-blur-sm rounded-xl p-3 text-center">
                ...
                <div className="text-xs text-purple-100">Entrées</div>
              </div>
              <div className="bg-white/20 backdrop-blur-sm rounded-xl p-3 text-center">
                <div className="text-2xl font-bold">{userStats.streakDays}</div>
                <div className="text-xs text-purple-100">Série</div>
              </div>
              <div className="bg-white/20 backdrop-blur-sm rounded-xl p-3 text-center">
                <div className="text-2xl font-bold">{userStats.completionRate}%</div>
                <div className="text-xs text-purple-100">Objectifs</div>
              </div>
            </div>
          )}
        </div>
              ...
              <div key={section.id}>
                <button
                  ...
                  className={`w-full glass-card backdrop-blur-sm rounded-2xl p-4 shadow-medium transition-all hover-lift ${
                    isActive ? 'border-2 border-purple-300 bg-purple-50/50' : 'border border-gray-200'
                  }`}
                >
        ...
        <div className="glass-card backdrop-blur-sm rounded-3xl p-6 shadow-large">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="bg-purple-50 rounded-2xl p-4 text-center border-2 border-dashed border-purple-300">
              <div className="text-3xl mb-2">🔒</div>
              <div className="font-bold text-purple-700">Prochaine</div>
              <div className="text-xs text-primary-600">Semaine parfaite</div>
            </div>
          </div>
        </div>
...
function ProfileSection({ userProfile, setUserProfile, avatarOptions, isEditing, setIsEditing }: {
  userProfile: UserProfile
...

{
  ...
  const [babyForm, setBabyForm] = useState(baby)
...
function SettingsSection({ settings, setSettings, colorSchemes }: {
  settings: AppSettings
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center space-x-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white ${
                    ...
                    member.role === 'grandparent' ? 'bg-purple-500' :
      ...
      <div className="glass-card backdrop-blur-sm rounded-2xl p-6 shadow-medium">
        ...
        <div className="space-y-3 text-sm ">
          ...
          <div className="pt-3 border-t border-gray-200">
            <button className="text-primary-600 hover:underline">Conditions d'utilisation</button>
            <span className="mx-2">•</span>
            <button className="text-primary-600 hover:underline">Politique de confidentialité</button>
          </div>
        </div>
      </div>
